...
function exportFile(){
	$(document).ready(function() {
	    ...
	    $.ajax({
				...
				type: 'POST',
				dataType: 'JSON',
				data: {'data_export': jsonString,'filename':'sale_month'},
				beforeSend : function() {
				...
				complete: function(){					
					window.location = '/streak_new/sale_month.xls';					
				}
		  });
...
function get_data_seniors()
{
	$(document).ready(function() {
		...
		$.ajax({
			url: "/streak_new/public/report/sale/get-data-seniors",		
			type: 'POST',
			dataType: 'JSON',
			...
			success: function(result, status){	
				...
				$('table.table-total tr.row-data').remove();
					jQuery.each( result.data, function( key, val ) {
							...
							if(string_columns_choice == "")
						   {
							   $("<td><a onclick=import_columns('1','"+val['id_senior']+"')>"+key+"</a></td>").appendTo('table.table-total tr:last');    
						   }	
						   else
						   {
								$("<td><a onclick=import_columns('2','"+val['id_senior']+"')>"+key+"</a></td>").appendTo('table.table-total tr:last');    
						   }
							
							$("<td>"+val['total_campaigns']+"</td>").appendTo('table.table-total tr:last');
							$("<td>"+val['total_actual_sold']+"</td>").appendTo('table.table-total tr:last');
...
function import_columns(level,id_senior)
 {
	 ...
	 $(document).ready(function() {
		...
		{	
		 ...
		 $.ajax({
				url: "/streak_new/public/report/sale/process-ajax-request",		
				type: 'POST',
				dataType: 'JSON',
				data: {'date_filter':date_filter,'input_filter':input_filter,'sortaction':sortaction,'id_senior':id_senior},
				beforeSend : function() {
